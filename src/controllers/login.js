...
form.addEventListener("submit", function (e) {
  ...
  if (email.value === "" || password.value === "") {
    mensaje.innerHTML =
      "<div class='alert alert-danger' role='alert'> ⚠️Por favor, complete todos los campos.</div>";
    return;
  }
  /* Búsqueda del usuario en la lista */
  const usuarioEncontrado = usuarios.find(
    (usuario) =>
      usuario.correo === email.value && usuario.contraseña === password.value,
  );
  ...
  if (usuarioEncontrado) {
    ...
    mensaje.innerHTML =
      "<div class='alert alert-danger' role='alert'>❌Correo electrónico o contraseña incorrectos.</div>";
  }
});
